...
<table id="Table_01" width="1025" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td colspan="6" rowspan="7" valign="top" background="images/proyek_15.gif"><p>&nbsp;</p>
      <h1 align="center" class="style5">Pesan</h1>
	        <?php 
	     $data     = mysql_query("SELECT * FROM user us inner join data_mahasiswa ds on us.nim=ds.nim WHERE username= '$_SESSION[username]'");
  		 $hasil     = mysql_fetch_array($data); ?>
		<form name='chat' action='pesan_dosen.php' method="post" enctype="multipart/form-data">
		<input type="hidden" name="TbSimpan" value="<?php echo $hasil['nim']; ?> " />Nim	: 
		<input type="text" name="TbSimpan" value="<?php echo $hasil['nim']; ?> " disabled="disabled" />
